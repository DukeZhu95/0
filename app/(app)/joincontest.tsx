...
//   const { contestId } = useLocalSearchParams<Params>();
...
//   // Effect for requesting permissions and loading music
//   useEffect(() => {
//     (async () => {
...
//         // // Director Mode: Switch between front and back cameras every 5 seconds
//         // let switchInterval: ReturnType<typeof setInterval> | null = null;
  
//         // // **New approach: Simulate camera switching**
//         // if (isDirectorMode) {
//         //   switchInterval = setInterval(async () => {
//         //     if (cameraRef.current && isRecording) {
//         //       try {
//         //         console.log("Stopping recording to switch camera...");
  
//         //         // **Stop current recording**
//         //         await cameraRef.current.stopRecording();
  
//         //         // **Simulate a switch effect (Optional)**
//         //         setFacing((prev) => (prev === "back" ? "front" : "back"));
//         //         await new Promise((resolve) => setTimeout(resolve, 1000)); // Short delay
  
//         //         console.log("Restarting recording after switch...");
//         //         await startRecording(); // **Restart recording immediately**
//         //       } catch (error) {
...
//       const cachePath = `${FileSystem.cacheDirectory}file_${Date.now()}.${extension}`;
...
//   //   Animated.timing(animationProgress, {
//   //     toValue: 1.5, // Zoom to 1.5x
//   //     duration: 2000, // 2 seconds smooth zoom
//   //     useNativeDriver: false,
//   //   }).start(() => {
//   //     Animated.timing(animationProgress, {
...
//   //       // **Play sound immediately after selecting**
...
//   // **Adjusted pickFromGallery Function**
...
//   //   // Navigate to PostContest Function
//   const navigateToPostContest = () => {
//     if (videoUri && contestId) {
//       router.push({
//         pathname: "/postcontest",
//         params: { videoUri, contestId }, // Pass both videoUri and contestId
//       });
//     } else {
//       Alert.alert(
//         "Error",
//         "No video selected or contest information missing. Please select a video and ensure you are within a contest."
//       );
//     }
//   };
...
//   // Toggle video playback by icon
//   //   const toggleVideoPlayback = async () => {
//   //     if (!status.isLoaded) return;
//   //     if (status.didJustFinish) {
//   //       // If finished playing, seek to the start before playing again
...
const JoinContest: React.FC = () => {
  ...
  //       try {
  //         setIsRecording(true);
  //         resetTimer(); // **Reset timer before starting a new recording**
  //         startTimer();
  //         Animated.timing(animationProgress, {
  //           toValue: 1,
  ...
  //         const videoRecordPromise = cameraRef.current.recordAsync({
  //           maxDuration: recordingTime, // **Set maxDuration to 180 seconds (3 minutes)**
  //           quality: "720p",
  //         });
  ...
  //           // **Automatically stop recording by updating state**
  //           setIsRecording(false);
  //           stopTimer();
  //           resetAnimation();
  //           resetTimer();
  ...
  //         stopTimer();
  //         resetAnimation();
  //         resetTimer(); // **Reset timer upon error to prevent lingering time**
  //       }
  //     }
  //       };
  ...
  //       if (selectedSound) {
  //         await soundRef.current.unloadAsync(); // Unload any previous sound
  //         await soundRef.current.loadAsync({ uri: selectedSound.uri });
  //         await soundRef.current.playAsync();
  //       }
  //       const videoRecordPromise = cameraRef.current.recordAsync({
  ...
  //         mute: selectedSound !== null, // Record with sound
  //       });
  ...
  const saveToCache = async (uri: string): Promise<string | null> => {
    try {
      ...
      const cachePath = `${FileSystem.cacheDirectory}file_${Date.now()}.${extension}`;
       ...
       // For iOS, ensure we're copying from the correct path
       const sourcePath = Platform.OS === 'ios' ? uri.replace('file://', '') : uri;
